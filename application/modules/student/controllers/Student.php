...
class Student extends MY_Controller {
  ...
  public function delet($id)
  {
    ...
    $data = ['id' => $id];
    $target = $this->m_student->id($data)->row();
    if ($target->ijasah != NULL) {
      unlink("./assets/ijazah/$target->ijasah");
    }
    if ($target->skl != NULL) {
      unlink("./assets/skl/$target->skl");
    }
  ...
  function do_upload() {
    ...
    //
    $config['file_name']     = $this->input->post('name')."-ijz-".date('dd/mm/yyyy');
  ...
  function skl_upload() {
    ...
    $src_file_name = 'skl';
    //
    $config['file_name']     = $this->input->post('name')."-skl-".date('dd/mm/yyyy');
    $config['upload_path']   = 'assets/skl/';
    $config['allowed_types'] = 'pdf';
    $config['max_size']      = 1024;
    //
    $this->upload->initialize($config);
    if(!empty($src_file_name)){
      if ($this->upload->do_upload($src_file_name)){
        ...
        //
        $result = $file_nm;
        $lspdf = $this->input->post('last-skl');
        if ($lspdf != "") {
          unlink("./assets/skl/$lspdf");
        }
      }else{
        $result = $this->input->post('last-skl');
      }
      //
      return $result;
    }   
  ...
  public function delet_by_class()
  {
    if ($this->access->_delet == 0) {
      $this->session->set_flashdata('cus', 'You do not have access to delete this data !');
      redirect('student');
